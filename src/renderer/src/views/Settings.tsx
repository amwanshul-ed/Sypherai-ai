...
const SettingsView = ({ isSystemActive }: SettingsProps) => {
  ...
  const [voice, setVoice] = useState<'MALE' | 'FEMALE'>(
    (localStorage.getItem('sypher_voice_profile') as 'MALE' | 'FEMALE') || 'MALE'
  )
  const [personality, setPersonality] = useState('')
  const [userName, setUserName] = useState(localStorage.getItem('sypher_user_name') || '')

  const [geminiKey, setGeminiKey] = useState(localStorage.getItem('sypher_custom_api_key') || '')
  const [groqKey, setGroqKey] = useState(localStorage.getItem('sypher_groq_api_key') || '')
  const [hfKey, setHfKey] = useState(localStorage.getItem('sypher_hf_api_key') || '')
  const [tailvyKey, setTailvyKey] = useState(localStorage.getItem('sypher_tailvy_api_key') || '')
  ...
  const handleVoiceChange = (v: 'MALE' | 'FEMALE') => {
    ...
    setVoice(v)
    localStorage.setItem('sypher_voice_profile', v)
  }
  ...
  const saveUserName = () => {
    localStorage.setItem('sypher_user_name', userName)
    alert('User Designation Saved.')
  }

  const saveApiKeys = async () => {
    localStorage.setItem('sypher_custom_api_key', geminiKey)
    localStorage.setItem('sypher_groq_api_key', groqKey)
    localStorage.setItem('sypher_hf_api_key', hfKey)
    localStorage.setItem('sypher_tailvy_api_key', tailvyKey)
  ...
  const startFaceEnrollment = async () => {
    ...
    try {
      ...
      enrollTimeoutRef.current = setTimeout(() => {
        ...
        setEnrollStatus('NO FACE DETECTED â€” CANCEL & RETRY')
      }, 30000)
      ...
      enrollIntervalRef.current = setInterval(async () => {
        ...
        try {
          ...
          setEnrollStatus('SCAN ERROR â€” RETRYING')
        }
      }, 700)
    } catch (e: any) {
      stopEnrollment()
