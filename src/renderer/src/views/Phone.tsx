...
const PhoneView = ({ glassPanel }: { glassPanel?: string }) => {
  const [ip, setIp] = useState(() => localStorage.getItem('sypher_adb_ip') || '')
  const [port, setPort] = useState(() => localStorage.getItem('sypher_adb_port') || '5555')
  ...
  const handleManualConnect = () => {
    localStorage.setItem('sypher_adb_ip', ip)
    localStorage.setItem('sypher_adb_port', port)
