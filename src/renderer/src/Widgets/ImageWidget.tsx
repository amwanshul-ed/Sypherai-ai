...
export default function ImageWidget() {
  ...
  useEffect(() => {
    const handleEvent = (event: any) => {
      ...
      if (retry) {
        setRetryInfo(retry)
        const modelTag = retry.model ? ` · ${String(retry.model).toUpperCase()}` : ''
        setStatusText(`ROTATING ENGINE${modelTag} · ATTEMPT ${retry.attempt}/${retry.total}`)
        return
      }
      ...
      if (loading) {
        ...
        setImageSrc('')
        setSavedPath('')
        setStatusText('CRAFTING NEURAL VISION…')
        return
      }
      ...
      if (url) {
        ...
        setStatusText(savedPath ? 'SAVED TO GALLERY' : 'IMAGE READY — GALLERY SAVE FAILED')
      }
    }
  ...
  const openSettings = () => {
    close()
    window.dispatchEvent(new CustomEvent('iris-open-settings'))
  }

  if (phase === 'idle') return null

  const truncatedPrompt = prompt.length > 70 ? `${prompt.slice(0, 70)}…` : prompt
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-[11px] font-black tracking-[0.25em] text-zinc-100 uppercase truncate">
                  Sypher AI · Visual Forge
                </span>
                <span className="text-[9px] font-mono tracking-widest text-emerald-500/60 uppercase truncate">
                  {phase === 'loading' &&
                    (retryInfo
                      ? `ENGINE ${retryInfo.model?.toUpperCase() || 'ROTATE'} · ${retryInfo.attempt}/${retryInfo.total}`
                      : 'NEURAL SYNTHESIS ACTIVE')}
                  {phase === 'success' && (savedPath ? 'ARCHIVED · GALLERY SYNCED' : 'RENDER COMPLETE')}
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-2 text-center max-w-md">
                    <span className="text-xs font-black tracking-[0.3em] text-emerald-300 uppercase">
                      {statusText || 'CRAFTING NEURAL VISION…'}
                    </span>
                    {retryInfo && (
