  ...
  useEffect(() => {
    const verifyAccess = async () => {
      try {
        ...
        if (!accessToken && !localStorage.getItem('sypher_cloud_token')) {
...
const PublicRoute = ({ children }: { children: JSX.Element }) => {
  ...
  const accessToken =
    useAuthStore((state) => state.accessToken) || localStorage.getItem('sypher_cloud_token')
  ...
  useEffect(() => {
    if (electronAPI) {
      electronAPI.on('oauth-callback', (_event: any, url: string) => {
        try {
          const urlObj = new URL(url.replace('sypher://', 'http://localhost/'))

          const refreshToken = urlObj.searchParams.get('refreshToken')
          const accessToken = urlObj.searchParams.get('accessToken')

          if (refreshToken && accessToken) {
            localStorage.setItem('sypher_cloud_token', refreshToken)
