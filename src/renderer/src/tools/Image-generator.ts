...
export const handleImageGeneration = async (prompt: string) => {
  ...
  try {
    const HF_API_KEY = localStorage.getItem('sypher_hf_api_key') || ''
    ...
    if (shouldUseFallback(errorMessage)) {
      try {
        ...
        errorMessage = `Fallback engine failed: ${fbErr?.message || String(fbErr)}. Try again in a moment, or add a Hugging Face key in Settings â†’ API Keys for FLUX.`
      }
    }
