  ...
  try {
    ...
    if (result.success) {
      ...
      const msg = result.wasResumed
        ? `âœ… Successfully resumed and completed ingestion. Memory Banks fully loaded.`
        : `âœ… Successfully ingested directory. Generated ${result.totalChunks} vectors.`
      return msg
    }
    return `âŒ Ingestion Aborted or Failed: ${result.error}`
  } catch (error) {
...
export const consultOracle = async (query: string): Promise<string> => {
  try {
    ...
    return `âŒ AI failed: ${result.error}`
  } catch (error) {
    return `âŒ System failure: ${String(error)}`
  }
}
