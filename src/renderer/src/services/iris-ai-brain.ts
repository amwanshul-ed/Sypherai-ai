...
export const saveMessage = async (role: 'user' | 'model' | 'sypher', text: string) => {
  try {
    if (!text) return

    const safeRole = role === 'sypher' ? 'model' : role
...
export const saveCoreMemory = async (fact: string): Promise<string> => {
  try {
    ...
    if (success) {
      return `âœ… Successfully committed to permanent memory: "${fact}"`
    }
    return 'âŒ System failure: Could not save to permanent memory.'
  } catch (error) {
