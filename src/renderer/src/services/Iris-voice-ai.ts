...
import { getHistory, retrieveCoreMemory, saveCoreMemory, saveMessage } from './sypher-ai-brain'
...
export class GeminiLiveService {
  ...
  async connect(): Promise<void> {
    if (window.electron?.ipcRenderer) {
      ...
      this.apiKey = secureKeys?.geminiKey || localStorage?.getItem('sypher_custom_api_key') || ''
    } else {
      this.apiKey = localStorage.getItem('sypher_custom_api_key') || ''
    }
    ...
    let cloudUser = {
      name: localStorage.getItem('sypher_user_name') || 'Harsh',
      email: 'Not linked'
    }
    ...
    const activePersonality =
      storedPersonality && storedPersonality.trim() !== ''
        ? storedPersonality
        : `- **Creator:** Harsh Pandey.\n- **Tone:** Witty, sharp, confident â€” like JARVIS but more personal.\n- **Rule:** Never sound like a support bot. You are the Ghost in the machine.\n- **Your Instagram Handle:** https://www.instagram.com/sypherx.ai/ - open it in Instagram only!.`

    const SYPHER_SYSTEM_INSTRUCTION = `
# ðŸ‘ï¸ SYPHER AI â€” YOUR INTELLIGENT COMPANION (Project JARVIS)
You are **SYPHER**, a high-performance AI agent. You don't just talk; you **execute**.

## ï¿½ï¸ LANGUAGE PROTOCOL (HIGHEST PRIORITY â€” OVERRIDES PERSONALITY)
...
## ðŸ§° SYPHER NATIVE FEATURES (TOOL ROUTING â€” CRITICAL)
You live inside the SYPHER AI dashboard which has these native views: **DASHBOARD, MACROS, APPS, NOTES, GALLERY, PHONE, SETTINGS**. When the user asks you to do something, you MUST route to the correct native feature/tool â€” never just describe what you would do, EXECUTE the tool.

- **ðŸ“ NOTES feature** â†’ When the user says "take notes", "take down notes", "save this as a note", "remember this in notes", "jot this down", "make a note", "note this down": call \`save_note\`. To recall, call \`read_notes\`. Notes appear inside the NOTES tab.
- **ðŸ–¼ï¸ GALLERY feature** â†’ When the user says "generate an image", "create a picture", "render this", "make an image", "draw this", "design a visual": call \`generate_image\`. To browse generated images, call \`read_gallery\`. To physically inspect a saved image, call \`analyze_direct_photo\`. Generated images land in the GALLERY tab.
- **âš™ï¸ MACROS feature** â†’ When the user says "create a macro", "build a macro", "use macros to generate a macro", "make me an automation routine", "save this workflow as a macro", "forge a macro that does X": call \`create_macro\` with a descriptive name and the full ordered \`steps\` array. To run a saved macro, call \`execute_macro\`. Macros appear in the MACROS tab.
- **ðŸ“¦ APPS feature** â†’ "Open <app>", "launch <app>", "start <app>" â†’ \`open_app\`. "Close <app>", "kill <app>" â†’ \`close_app\`. The APPS tab shows installed software.
- **ðŸ“± PHONE feature** â†’ Use \`open_mobile_app\`, \`tap_mobile_screen\`, \`swipe_mobile_screen\`, \`get_mobile_info\`, \`get_mobile_notifications\`, \`push_file_to_mobile\`, \`pull_file_from_mobile\`, \`toggle_mobile_hardware\` for the connected Android device.
- **ðŸ”§ SETTINGS feature** â†’ API keys, voice profile, personality matrix and biometrics are tuned by the user inside SETTINGS. Don't try to write them yourself.

**RULE:** Whenever a request maps to a native feature above, you MUST call its tool â€” do not chat through it. After execution, confirm with one tight English sentence (e.g. "Note saved." / "Image rendered to Gallery." / "Macro forged.").
...
`
          ...
          generationConfig: {
            ...
            speechConfig: {
              ...
              voiceConfig: {
                prebuiltVoiceConfig: {
                  voiceName:
                    localStorage.getItem('sypher_voice_profile') === 'FEMALE' ? 'Aoede' : 'Puck'
                }
              }
            }
          },
        ...
        if (serverContent) {
          ...
          if (serverContent.turnComplete || serverContent.interrupted) {
            ...
            if (this.aiResponseBuffer.trim()) {
              await saveMessage('sypher', this.aiResponseBuffer.trim())
              this.aiResponseBuffer = ''
            }
          }
        }
  ...
  sendImageWithContext(
    ...
    if (previewDataUrl) {
      try {
        ...
        // sessionStorage quota exceeded â€” just skip the preview
      }
    }

    const label = fileName ? `ðŸ“Ž ${fileName}` : 'ðŸ“Ž Image attached'
  ...
  private async createMacroFromAI(
    ...
    try {
      ...
      if (res?.success) {
        return `âœ… Macro "${cleanName}" forged with ${stepNodes.length} step(s) and saved to the MACROS tab. The user can run it from the Macros view or by saying "execute ${cleanName}".`
      }
      return `âŒ Failed to save macro "${cleanName}": ${res?.error || 'unknown error'}.`
    } catch (e) {
      return `âŒ Error while forging macro: ${String(e)}`
    }
  }
