...
export default function registerFileSearch(ipcMain: IpcMain) {
  ipcMain.handle('index-folder', async (event, folderPath: string) => {
    try {
      ...
      const dbPath = path.join(app.getPath('userData'), 'sypher_semantic_db')
      ...
      return `âœ… Successfully indexed ${filesToIndex.length} files.`
    } catch (err) {
      return `âŒ Indexing Error: ${String(err)}`
    }
  })
  ...
  ipcMain.handle('search-files', async (event, { query, groqKey }) => {
    try {
      ...
      return `âŒ System Error: ${String(err)}`
    }
  })
}
