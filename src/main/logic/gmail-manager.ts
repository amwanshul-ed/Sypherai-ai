...
export default function registerGmailHandlers(ipcMain: IpcMain) {
  ...
  ipcMain.handle('gmail-read', async (_event, maxResults = 5) => {
    try {
      ...
      if (!messages.length) return { speechText: prefix + 'ðŸ“­ Inbox is empty.', uiData: [] }
      ...
      for (const msg of messages) {
        ...
        emailListForSypher.push(`ðŸ“§ From: ${from}\nSubject: ${subject}\nPreview: ${snippet}\n`)
      ...
      return {
        speechText: prefix + emailListForSypher.join('\n---\n'),
        uiData: uiDataArray
      }
    } catch (e: any) {
      return { speechText: `âŒ Gmail Error: ${e.message}`, uiData: [] }
    }
  })
  ...
  ipcMain.handle('gmail-send', async (_event, { to, subject, body }) => {
    try {
      ...
      return prefix + `âœ… Email successfully sent to ${to}.`
    } catch (e: any) {
      return `âŒ Send Error: ${e.message}`
    }
  })
  ...
  ipcMain.handle('gmail-draft', async (_event, { to, subject, body }) => {
    try {
      ...
      const prefix = isNewLogin ? '[SYSTEM NOTICE: Login successful.]\n\n' : ''
      return prefix + `âœ… Draft created for ${to}. You can review it in your Gmail.`
    } catch (e: any) {
      return `âŒ Draft Error: ${e.message}`
    }
  })
}
