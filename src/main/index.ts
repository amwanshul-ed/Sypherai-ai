...
const secureConfigPath = join(app.getPath('userData'), 'sypher_secure_vault.json')
...
app.on('second-instance', (event, commandLine) => {
  ...
  if (mainWindow) {
    ...
    mainWindow.focus()
    const url = commandLine.find((arg) => arg.startsWith('sypher://'))
    if (url) {
...
app.whenReady().then(() => {
  ...
  app.on('open-url', (event, url) => {
    ...
    if (mainWindow && url.startsWith('sypher://')) {
      mainWindow.webContents.send('oauth-callback', url)
    }
  })
